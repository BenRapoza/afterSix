...
export default async function ExploreTonight() {
  ...
  return <main className={styles.page}><nav className={`${styles.nav} ${mobileStyles.nav}`}><Link href="/">afterSix</Link><Link href="/planner">Build your night</Link></nav><section className={`${styles.content} ${mobileStyles.content}`}><p className={styles.kicker}>LIVE BOSTON DISCOVERY</p><h1>What&apos;s happening<br/><em>tonight?</em></h1><p className={`${styles.date} ${mobileStyles.date}`}>{date}</p><div className={`${styles.grid} ${mobileStyles.grid} glass-grid`}>{items.map(([icon, count, label]) => <article key={label}><span>{icon}</span><b>{count}</b><p className="explore-event-label">{label}</p></article>)}</div><Link className={`${styles.cta} ${mobileStyles.cta} glass-cta`} href="/planner?random=1">Build me a date around this →</Link><p className={`${styles.note} ${mobileStyles.note}`}>Live counts from Ticketmaster and Google Places.</p></section></main>;
}
