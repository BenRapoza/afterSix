...
export async function GET() {
  ...
  const { data, error } = await getSupabaseAdmin().from("saved_nights").select("id, title, created_at").eq("clerk_user_id", userId).order("created_at", { ascending: false }).limit(20);
...
export async function POST(request: Request) {
  ...
  const { error } = await getSupabaseAdmin().from("saved_nights").insert({ clerk_user_id: userId, title: input.title ?? "Moonlit Boston", itinerary: input.itinerary });
